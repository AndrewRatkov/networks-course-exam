...
EDC -- Error Detection and Correction -- биты для обнаружения ошибок.
...
CSMA/CD + collision detection. Передача, при которой возникла коллизия, отменяется. Обнаруживать коллизии просто в кабельных ЛВС, но в беспроводных сложно.
...
Коммутатор (свич) -- снижает количество коллизий, промежуточно может хранить пакеты
...
## Виртуализация каналов: MPLS. Сравнение путей MPLS и IP.
Многопротокольная коммутация по меткам.

Хотим ускорить коммутацию по IP с использованием меток.
Структура кадра:

PPP/Ethernet заголовок -- MPLS заголовок -- IP заголовок -- остаток кадра

При IP-маршрутизации путь зависит только от целевого адреса.

MPLS-маршрутизация: путь зависит от начального и от конечного адреса, путей может быть несколько, резервные тоже рассчитываются для быстрой перемаршрутизации.

Пути вычисляются на основе измененного OSPF.

## Сети для дата-центров. Балансировщик нагрузки. Иерархия маршрутизаторов и коммутаторов. Полносвязная топология.
Проблема: много клиентов => надо умело распределять нагрузку.

Балансировщик нагрузки занимается маршрутизацией на прикладном уровне. Он получает все запросы от клиентов , перенаправляет их равномерно на сервера дата-центра. Затем через него возвращаются результаты клиенту.

Для повышенной пропускной спрособности есть:
- стойки
- коммутаторы 2 уровня
- надстоечные коммутаторы

![screen09.png](big_topology.png)

## Один день из жизни веб-запроса: протоколы (на всех уровнях), применяемые при запросе одной веб-страницы.

Студент выходит с ноутбука в локальную сеть факультета, запрашивает и получает страницу www.google.com.

1) Получение IP-адреса. DHCP-запрос (-> UDP --> IP --> 802.3 Ethernet). Ethernet-кадр широковещается в ЛВС, принимается на маршрутизаторе с DHCP-сервером. Там ответ демультиплексируется, DHCP-сервер формирует клиентский IP-адрес, и отсылает DHCP ACK, в котором также содержится информация о первом транзитном маршрутизаторе и локальном DNS-сервере. Клиент получает DHCP ACK.

2) Достучаться до первого транзитного маршрутизатора (перед отправкой DNS-запроса). Клиент уже знает его IP-адрес, но не знает MAC-адрес, его надо получить. Для этого он отправляет широковещательно ARP-запрос, маршрутизатор отвечает на него ARP-откликом. 

3) Отправить DNS-запрос (--> UDP --> IP). IP-дейтаграмма, содержащая запрос, оказывается на первом транзитном маршрутизаторе, и затем пересылается на основе таблиц, созданных с помощью RIP/OSPF/BGP на DNS-сервер. Он отвечает IP-адресом страницы www.google.com.

4) Отправить HTTP-запрос (--> TCP --> IP). Клиент сначала открывает TCP-сокет для соединения. Отправиляет SYN-сегмент, получает SYNACK-сегмент (первые 2 этапа тройного рукопожатия).Отправляет GET-запрос в TCP-сегменте. Веб-сервер отвечает HTTP-откликом.

## Беспроводные и мобильные сети. Основные элементы беспроводной сети. Таксономия беспроводных сетей. Отличия от проводных каналов связи.

Элементы беспроводной сети:
- беспроводные хосты, могут быть мобильными или стационарными
- базовые станции, подключены к проводной сети. *Реле* -- отвечает за обмен пакетами между проводной и беспроводной сетью.
- беспроводной канал (для подключения мобитьного узла к станции). Может иметь разные скорости и дальности передачи данных.

Мобильные узлы могут менять свою базовую станцию.

Также возможен режим прямого подключения -- когда узлы могут передавать данные узлам в небольщой зоне покрытия.

Таксономия:
- однопереходные
    - без инфраструктуры (Bluetooth), нет выхода в Интернет
    - с инфраструктурой (Wi-fi, сотовая связь), есть подключение к базовой станции
- многопереходные
    - без инфраструктуры (MANET, VANET), для доступа к узлу используется посредник
    - с инфраструктурой (ячеистая сеть), для доступа в Интернет данные проходят через несколько беспроводных узлов

Отличия от проводных каналов связи:
- ослабление сигнала
- многолучевое распространение
- помехи от других источников

## Характеристики беспроводной сети. SNR, BER. 

- SNR (signal-to-noise ratio) -- соотношение сигнал-шум.
- BER (bit-error-rate)

На физическом уровне: увеличение мощности => увеличивается SNR => уменьшается BER. 

![screen10.png](snr2ber.png)

BPSK(1 Мбит/с), QAM16(4 Мбит/с), QAM256(8 Мбит/с).

- проблема скрытых передатчиков (Б и А пеленгуют друг друга, В и Б пеленгуют друг друга, А и В не знают, что создают помехи Б)
- ослабление сигнала, когда А и В не пеленгуют друг друга, а через Б.

## Множественный доступ с кодовым разделением (CDMA). Кодирование / декодирование. Пример.

У каждого пользователя уникальный код.

![screen11.png](encoding_decoding.png)

## Беспроводные локальные сети IEEE 802.11. Пассивное/активное сканирование.
- 802.11a: 5-6 ГГц, до 54 Мбит/с, у всех хостов одинаковый отсекающий код.
- 802.11b: 2.4-5 Ггц, до 11 Мбит/с.
- 802.11g: 2.4-5 Ггц, до 54 Мбит/с.
- 802.11n: 2.4-5 Ггц, до 200 Мбит/с -- несколько антенн.

ОНС (основной набор служб, сота)

Подкдичение хоста к ТД: сканирование каналов, пеленгование кадров, содержащих имя (SSID) и MAC-адрес точки доступа.

- Пассивное сканирование: когда подключение начинается с сигнальных кадров от точек доступа (регулярно ими рассылаются)
- Активное сканирование: когда подключение начинается с пробного запроса от хоста к всем ОНС.

## МАС протокол IEEE 802.11: CSMA/CA (collision avoidance). Избежание столкновений.

802.11: CSMA --- прослушка канала перед трансляцией, чтобы избежать столкновений.

Идея: отправитель может резервировать канал для данных. Для этого он отправляет запрос в кадре RTS для отправки кадров и, если всё хорошо, получает ответ в кадре CTS, который пеленгуется со всеми узлами. Все кроме отправителя откладывают передачу данных. 
